...
async function wakeSet(street, radiusFt) {
  ...
  return { subject: subj, comps: comps, source: "Wake County parcel record (the county's own GIS)" };
}
...
async function meckSet(street, radiusFt) {
  ...
  return { subject: subj, comps: comps,
           source: "Mecklenburg County CAMA parcel data (the county's own GIS)" };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let set = null;
    for (const radius of [2000, 5280]) {
      if (C.indexOf("WAKE") === 0) set = await wakeSet(street, radius).catch(() => null);
      else if (C.indexOf("MECKLENBURG") === 0) set = await meckSet(street, radius).catch(() => null);
      else set = await stateSet(street, cityU, C, radius, tenant).catch(() => null);
      if (set && (set.comps || []).length >= 6) break;
      if (set && radius === 5280) break;
      if (!set) break;
    }
    ...
    // size band first, and only widen if it empties the list
    if (subject.heatedArea) {
      const band = comps.filter(r => r.heatedArea &&
        r.heatedArea >= subject.heatedArea * 0.65 && r.heatedArea <= subject.heatedArea * 1.45);
