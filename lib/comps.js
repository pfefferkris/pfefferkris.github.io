...
const CUMB_SCHOOL = "https://gis.co.cumberland.nc.us/server/rest/services/BoardofEducation/SchoolDistricts/MapServer";
const CUMB_SALES = "https://gis.co.cumberland.nc.us/server/rest/services/Tax/ParcelSales/MapServer";
// Residential Sales, one layer per year, most recent first
const CUMB_SALES_LAYERS = [13, 14, 15, 16, 17];
...
async function schoolZone(county, c) {
  ...
  /* Cumberland publishes the same three levels off its Board of Education service, so the
     school-zone method runs on real assignment here too, not on proximity. */
  if (String(county || "").toUpperCase().indexOf("CUMBERLAND") === 0) {
    const out = { source: "Cumberland County Schools attendance boundaries", assigned: true };
    const lv = [[0, "elementary"], [1, "middle"], [2, "high"]];
    for (const [id, level] of lv) {
      try {
        const j = await q(CUMB_SCHOOL + "/" + id + "/query",
          Object.assign({}, base, { outFields: "School,Grades" }));
        const a = ((j.features || [])[0] || {}).attributes || {};
        if (str(a.School)) out[level] = str(a.School);
      } catch (e) {}
    }
    return (out.elementary || out.middle || out.high) ? out : null;
  }
...
/* Cumberland. The only county table in the state that carries bathroom counts, which is
   the difference between running his adjustment rules and running two thirds of them.
   The sale is the package sale on the parcel record, and the excise stamp is beside it, so
   every comparable can be checked against the consideration the parties swore to rather
   than a field somebody typed. */
const CUMB_FIELDS = "PIN,LOCATION_ADDR,CITY,HEATED_AREA,BEDROOMS,BATH_FULL,BATH_HALF," +
  "YEAR_BUILT,EFF_YEAR,GRADE,CONDITION,BLDG_DESC,ACREAGE,LAND_CLASS,NEIGHBORHOOD,VCS," +
  "PKG_SALE_PRICE,PKG_SALE_DATE,REVENUE_STAMPS,TOTAL_PROP_VALUE," +
  "TOTAL_LAND_VALUE_ASSESSED,TOTAL_BLDG_VALUE_ASSESSED,TOTAL_UNITS,STORY_HEIGHT";

async function cumbSet(street, radiusFt) {
  const s = street.replace(/'/g, "''");
  let sq = await q(CUMB, { where: "LOCATION_ADDR LIKE '" + s + "%'", outFields: CUMB_FIELDS,
    returnGeometry: "true", outSR: "4326", resultRecordCount: "1" });
  let f = (sq.features || [])[0];
  if (!f) {
    const t = s.split(" ");
    if (t.length > 2) {
      sq = await q(CUMB, { where: "PHYADDR_STR_NUM='" + t[0] + "' AND PHYADDR_STR LIKE '" +
        t.slice(1).join(" ") + "%'", outFields: CUMB_FIELDS,
        returnGeometry: "true", outSR: "4326", resultRecordCount: "1" });
      f = (sq.features || [])[0];
    }
  }
  if (!f) return null;
  const c = centroid(f.geometry);
  const subj = cumbRow(f.attributes, c, c);
  if (!c) return { subject: subj, comps: [] };

  /* The neighbours do NOT come from the parcel table. Its sale-price column is populated
     on only a minority of parcels — the last package sale, where one was ever entered —
     so a radius search against it returns two houses where a hundred changed hands. The
     county keeps a separate service of the sales themselves, one layer per year and per
     property class, and that is the real record: nearly two hundred residential sales
     inside a half mile of Haymount rather than two. Five years of it, queried at once. */
  const rings = await Promise.all(CUMB_SALES_LAYERS.map(function (id) {
    return q(CUMB_SALES + "/" + id + "/query", {
      geometry: JSON.stringify({ x: c.x, y: c.y, spatialReference: { wkid: 4326 } }),
      geometryType: "esriGeometryPoint", inSR: "4326", distance: String(radiusFt),
      units: "esriSRUnit_Foot", spatialRel: "esriSpatialRelIntersects",
      where: "PKG_SALE_PRICE > 10000 AND HEATED_AREA <> '0'",
      outFields: CUMB_FIELDS, returnGeometry: "true", outSR: "4326",
  ...
  rings.forEach(function (r) {
    (r.features || []).forEach(function (x) {
      const row = cumbRow(x.attributes, centroid(x.geometry), c);
      const k = row.pin + "|" + row.saleDate;      // one parcel can appear in two layers
      if (seen[k]) return;
      seen[k] = 1;
      comps.push(row);
    });
  ...
  return { subject: subj, comps: comps, origin: c,
           source: "Cumberland County tax parcel and recorded-sales services (the county's own GIS)" };
}
function cumbRow(a, c, origin) {
  const stamps = num(a.REVENUE_STAMPS);
  const stated = num(a.PKG_SALE_PRICE);
  const stampPrice = stamps ? Math.round(stamps * 500) : null;      // NCGS 105-228.30
  return {
    address: str(a.LOCATION_ADDR), pin: str(a.PIN), city: str(a.CITY),
    heatedArea: num(a.HEATED_AREA), bedrooms: num(a.BEDROOMS),
    bathsFull: num(a.BATH_FULL),
    // zero half-baths is a fact, not a missing value; num() folds 0 to null everywhere
    // else on purpose, and here that turns "two and no half" into "two and unknown"
    bathsHalf: a.BATH_HALF == null || a.BATH_HALF === "" ? null : (parseInt(a.BATH_HALF, 10) || 0),
    yearBuilt: num(a.YEAR_BUILT), effectiveYear: num(a.EFF_YEAR),
    grade: str(a.GRADE), condition: str(a.CONDITION),
    style: str(a.BLDG_DESC), stories: str(a.STORY_HEIGHT),
    acres: num(a.ACREAGE), use: str(a.LAND_CLASS),
    neighborhood: str(a.NEIGHBORHOOD), marketArea: str(a.VCS),
    units: num(a.TOTAL_UNITS),
    salePrice: stated, saleDate: iso(a.PKG_SALE_DATE),
    stamps: stamps, stampPrice: stampPrice,
    stampAgrees: (stated && stampPrice) ? Math.abs(stated - stampPrice) <= 500 : null,
    assessed: num(a.TOTAL_PROP_VALUE),
    land: num(a.TOTAL_LAND_VALUE_ASSESSED), building: num(a.TOTAL_BLDG_VALUE_ASSESSED),
    miles: milesBetween(origin, c)
  };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const radius of [1320, 2640, 5280, 10560]) {
      ...
      else if (C.indexOf("CUMBERLAND") === 0) got = await cumbSet(street, radius).catch(() => null);
    ...
    /* Then bathrooms — third in his order, and only reachable where the county publishes
       them, which today is Cumberland alone. A tightening that would leave fewer than
       three is not applied, because three comparables you matched is worth more than five
       you filtered down to by accident. */
    if (subject.bathsFull) {
      const subjB = subject.bathsFull + (subject.bathsHalf || 0) * 0.5;
      const bt = comps.filter(r => r.bathsFull == null ||
        Math.abs((r.bathsFull + (r.bathsHalf || 0) * 0.5) - subjB) <= 1);
      if (bt.filter(r => r.salePrice).length >= 3) comps = bt;
    }
    /* Same assessor neighbourhood beats a nearer house across the line, because the line is
       usually there for a reason the map does not show. Only if enough of them sold. */
    if (subject.neighborhood) {
      const same = comps.filter(r => r.neighborhood === subject.neighborhood);
      if (same.filter(r => r.salePrice).length >= 3) comps = same;
    }
    ...
    const out = {
      ...
      method: "Selected on square footage first, then bedroom count" +
              (subject.bathsFull ? ", then bathrooms" : "") +
              (subject.neighborhood ? ", then the assessor's own neighbourhood" : "") +
              ", then distance and how recent the sale is. Three to five, never more: past " +
              "five you are widening the net rather than sharpening the match." +
              (subject.bathsFull ? "" : " This county does not publish bathroom counts, so " +
    ...
    /* Where the stamp is published, every price is checkable, and a line where the stated
       price and the stamp disagree is a line to look at rather than to average. */
    const checked = sold.filter(r => r.stampAgrees !== null && r.stampAgrees !== undefined);
    if (checked.length) {
      const off = checked.filter(r => r.stampAgrees === false);
      out.stampCheck = { checked: checked.length, disagree: off.length };
      if (off.length) out.limits.push(off.length + " of " + checked.length + " sales here have a " +
        "stated price that does not match the excise stamp on the deed. That usually means the " +
        "stamp covered more than the one parcel, or part of the transfer was exempt. Those lines " +
        "are worth opening before you lean on them.");
    }
